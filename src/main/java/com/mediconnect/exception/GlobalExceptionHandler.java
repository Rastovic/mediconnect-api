...
public class GlobalExceptionHandler {
    ...
    // [A02] EntityNotFoundException handler exposes two internal infrastructure details:
    //
    ...
    public ResponseEntity<Map<String, Object>> handleEntityNotFound(EntityNotFoundException ex) {
        ...
        body.put("error",     ex.getMessage());   // [A02] "Entity with ID 99 not found in table medical_records"
        body.put("id",        ex.getId());         // [A02] raw PK exposed
        body.put("table",     ex.getTableName());  // [A02] DB table name exposed
        body.put("timestamp", LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(body);
    }

    // [A02] Catch-all for RuntimeException — forwards ex.getMessage() verbatim.
    //
    ...
    public ResponseEntity<Map<String, Object>> handleRuntime(RuntimeException ex) {
        Map<String, Object> body = new LinkedHashMap<>();
        body.put("error",     ex.getMessage());   // [A02] raw internal message forwarded
        body.put("type",      ex.getClass().getName()); // [A02] fully-qualified class name exposed
        body.put("timestamp", LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(body);
    }

    // [A02] Catch-all for any unhandled Throwable (including Error subclasses).
    ...
    public ResponseEntity<Map<String, Object>> handleThrowable(Throwable ex) {
        Map<String, Object> body = new LinkedHashMap<>();
        body.put("error",     ex.getMessage());         // [A02] JVM-level error message
        body.put("type",      ex.getClass().getName()); // [A02] e.g. "java.lang.OutOfMemoryError"
        body.put("timestamp", LocalDateTime.now());
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(body);
    }
}
